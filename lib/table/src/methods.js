...
var Methods = {
    ...
    if (!sortOpts.remote && defaultSort) {
      ...
      if (defaultSort.length) {
        defaultSort.forEach(function (item) {
          ...
          if (field && order) {
            var column = _this29.getColumnByField(field);
    ...
    if (sortConfs.length) {
      ...
      (multiple ? sortConfs : [sortConfs[0]]).forEach(function (confs) {
        ...
        if (_ctor.default.isString(field)) {
          column = _this30.getColumnByField(field);
        }
        ...
        if (column && (column.sortable || column.remoteSort)) {
          ...
          if (orders.indexOf(order) === -1) {
            order = getNextSortOrder(_this30, column);
          }
   ...
   */
  clearSort: function clearSort(column) {
    var sortOpts = this.sortOpts;

    if (column && _ctor.default.isString(column)) {
  ...
  reloadExpandContent: function reloadExpandContent(row) {
    ...
    if (lazy && expandLazyLoadeds.indexOf(row) === -1) {
      this.clearRowExpandLoaded(row).then(function () {
        return _this31.handleAsyncRowExpand(row);
      });
    }
  ...
  handleAsyncRowExpand: function handleAsyncRowExpand(row) {
    ...
    return new Promise(function (resolve) {
      _this32.expandLazyLoadeds.push(row);

      _this32.expandOpts.loadMethod({
        $table: _this32,
        row: row,
        ...
        _ctor.default.remove(_this32.expandLazyLoadeds, function (item) {
          return item === row;
        });

        _this32.rowExpandeds.push(row);
  ...
  setRowExpand: function setRowExpand(rows, expanded) {
    ...
    if (rows) {
      ...
      if (expanded) {
        validRows.forEach(function (row) {
          if (rowExpandeds.indexOf(row) === -1) {
            ...
            if (isLoad) {
              lazyRests.push(_this33.handleAsyncRowExpand(row));
            } else {
      ...
      if (reserve) {
        validRows.forEach(function (row) {
          return _this33.handleRowExpandReserve(row, expanded);
        });
      }
    }
  ...
  clearRowExpand: function clearRowExpand() {
    ...
    if (reserve) {
      tableFullData.forEach(function (row) {
        return _this34.handleRowExpandReserve(row, false);
      });
    }
  ...
  reloadTreeChilds: function reloadTreeChilds(row) {
    ...
    if (lazy && row[hasChild] && treeLazyLoadeds.indexOf(row) === -1) {
      this.clearTreeExpandLoaded(row).then(function () {
        return _this35.handleAsyncTreeExpandChilds(row);
      });
    }
    ...
    return new Promise(function (resolve) {
      ...
      loadMethod({
        ...
        if (childs) {
          ...
          _this36.appendTreeCache(row, childs);
          ...
          if (!checkStrictly && _this36.isCheckedByCheckboxRow(row)) {
            _this36.setCheckboxRow(childs, true);
          }
        }
  ...
  setTreeExpand: function setTreeExpand(rows, expanded) {
    ...
    if (rows) {
      ...
      if (rows.length) {
        ...
        if (expanded) {
          validRows.forEach(function (row) {
            if (treeExpandeds.indexOf(row) === -1) {
              ...
              if (isLoad) {
                result.push(_this37.handleAsyncTreeExpandChilds(row));
              } else {
        ...
        if (reserve) {
          validRows.forEach(function (row) {
            return _this37.handleTreeExpandReserve(row, expanded);
          });
        }
  ...
  clearTreeExpand: function clearTreeExpand() {
    ...
    if (reserve) {
      _ctor.default.eachTree(tableFullData, function (row) {
        return _this38.handleTreeExpandReserve(row, false);
      }, treeOpts);
    }
  ...
  computeScrollLoad: function computeScrollLoad() {
    ...
    return this.$nextTick().then(function () {
      var sYOpts = _this39.sYOpts,
          sXOpts = _this39.sXOpts,
          scrollXLoad = _this39.scrollXLoad,
          scrollYLoad = _this39.scrollYLoad,
          scrollXStore = _this39.scrollXStore,
          scrollYStore = _this39.scrollYStore; // 计算 X 逻辑

      if (scrollXLoad) {
        var _computeVirtualX3 = computeVirtualX(_this39),
      ...
      var _computeVirtualY = computeVirtualY(_this39),
    ...
    if (treeConfig) {
      ...
      if (matchObj) {
        ...
        nodes.forEach(function (row, index) {
          if (index < nodes.length - 1 && !_this41.isTreeExpandByRow(row)) {
            _this41.setTreeExpand(row, true);
          }
        });
      }
    }
   ...
   *************************/
  // 设置表格高度
  setHeightDc: function setHeightDc(_height) {
    this.height = _height;
  }
  /*************************
   * 自定义方法
